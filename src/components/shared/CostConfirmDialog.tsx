  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onCancel}>
      <div className="w-80 rounded-lg border border-stage-border bg-stage-base p-5" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-sm font-semibold text-stage-text">영상을 생성할까요?</h3>
        <dl className="mt-3 space-y-1 text-sm text-stage-text-muted">
          <div className="flex justify-between"><dt>필요 크레딧</dt><dd className="text-stage-text">{creditCost}</dd></div>
          <div className="flex justify-between"><dt>현재 잔액</dt><dd className={enough ? 'text-stage-text' : 'text-red-400'}>{balance}</dd></div>
        </dl>
        <p className="mt-3 text-xs text-stage-text-muted">생성 요청 시 크레딧이 차감돼요. 실패하면 자동 환불돼요.</p>
