...
interface Props {
  ...
  speaking?: boolean
  // 좌석 클릭 → 아바타 크게보기(빈 슬롯이면 미전달). UX-AVATAR-KBD: onClick 있으면 키보드 접근(Enter/Space).
  onClick?: () => void
  label?: string // 크게보기 대상(참가자명 등) — 키보드/SR aria-label
  children: ReactNode
}

// 무대 1석: 3×3 그리드 위치에 배치하고, 말하는 사람이면 앞 레이어로 강조한다.
// 좌석 콘텐츠(SelfAvatar·RemoteAvatar·빈자리)는 상위(Stage)가 넣는다 — 배치 책임만 여기 둔다.
export default function StageSlot({ col, row, speaking = false, onClick, label, children }: Props) {
  return (
    <div
      style={{ gridColumn: col, gridRow: row }}
      data-speaking={speaking ? 'true' : 'false'}
      onClick={onClick}
      // UX-AVATAR-KBD: 클릭 가능한 좌석은 키보드로도 조작 — role/tabIndex/Enter·Space.
