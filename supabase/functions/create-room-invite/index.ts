...
// ponytail: v1 은 role='actor' 고정 — viewer 초대는 뷰어 입장 경로(Phase 4)와 함께 개방.
...
Deno.serve(async (req) => {
  ...
  let body: { room_id?: unknown; max_uses?: unknown; expires_h?: unknown };
  try {
    body = await req.json();
  } catch {
  ...
  return json({ invite_code: code, room_id: room.id, max_uses: maxUses, expires_at: expiresAt }, 201);
});
