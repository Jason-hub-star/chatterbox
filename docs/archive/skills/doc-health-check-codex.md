...
# 내비 체인 규모 (작아야 건강)
for f in AGENTS.md docs/INDEX.md docs/status/PROJECT-STATUS.md; do printf "%5s줄  %s\n" "$(wc -l<"$f")" "$f"; done
...
| 기준 | 좋음 | 주의/나쁨 |
|---|---|---|
| **인덱싱성** | 내비 체인이 현재 작업(캐릭터/태스크)을 일관되게 가리킴, 날짜 최신 | INDEX/Codex가 stale(구 캐릭터를 "현재"로), 현재 작업 누락 |
...
흔한 조치:
- staleness: INDEX `Updated:` 날짜 + "현재 캐릭터/태스크" 행 갱신, Codex "Current Next Work" 최신화.
- 비대 SSOT: 날짜/단계 경계로 **분할 아카이브** — 삭제가 아니라 이동, `docs/archive/`로. 무손실 검증(메인 N행 + 아카이브 M행 = 원본). 참조 문서(INDEX/Codex/ARCHIVE-INDEX)와 상호참조도 함께 갱신.
