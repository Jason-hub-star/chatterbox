...
| `ReactionWheel.md` | `reactionStore.slots`, `reactionStore.floats` | `reactionStore.setSlots`, `reactionStore.addFloat`, `reactionStore.removeFloat` | `reaction` DataChannel(reliable, `{emoji}`). floats 휘발성·미영속, slots 는 localStorage 커스터마이즈 |
