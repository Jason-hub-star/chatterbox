...
콘텐츠 (위 → 아래, flex-column) — 2026-07-13 재편(텔레프롬프터 A안 "무대용 위계"·세션정보 카드 제거):
좁은 dock(clamp 216~256px)에서 **현재 대사가 지배 요소(≈70% 실측)**가 되도록 셋업 chrome 를 ⚙ 설정 뒤로 접는다.
1. **헤더 1줄** — 모드 태그(`리허설`/`본공연`·호스트=토글·비호스트=읽기전용 라벨) + 대본 제목(데이터 그대로).
2. **역할 칩** — 배역별 칩: 내 역할=앰버 채움(탭=내려놓기)·남=테두리·빈=파선(탭=맡기, 관전자는 정적).
3. **텔레프롬프터 포커스(★ 지배)** — 현재 대사(크게·긴 대사는 `max-h-[40vh]` 박스 내부 스크롤) + `내 차례` + **다음 대사 미리보기** + 진행 버튼. `features/script/TeleprompterFocus.tsx`(배치 무관 — C 무대 오버레이 승급 지점).
4. **⚙ 설정(접힘, default)** — 셋업 chrome: 개인 글자 크기 `A−/A+`(ROOM-06)·호스트 역할 배정/해제(ROOM-14)·전체 대본(현재 줄 하이라이트). 공연 중엔 숨어 대사에 초점.

폐기: ~~언어 선택 3-버튼 토글~~(미구현 — UI 언어는 전역 설정) · ~~세션 정보 카드(장르·러닝타임·녹음 포맷)~~(상단바 elapsed·#태그와 중복 → 2026-07-13 제거).
