...
| `eastern-plaza-1` | **동양(eastern) 월드 광장 허브** — 밤 범동양 홍등 광장, 가게 7 식별(쌍가면 대극장·드레스폼 의상실·셔터 예비점포·대계단 회관·망치모루 공방·찻잔 다관·목조 무대), 무글자·무인물·발광 잉어/고래·풍등·보름달 | eastern | **생성·배선 완료(2026-07-31)** — edits+**eastern_splash 레퍼런스**(화풍 고정)·gpt-image-2 175s → WebP 442KB `lobby-plaza/eastern-plaza-1.webp`. EASTERN Composition 자체 캘리브(가게 7·홍등 9·하늘 밴드, 원화 오버레이+실렌더 호버 검증) + `plazaMood:'night'` → `.hub-cloud--night`(screen, 픽셀 diff Δ16.6≈낮 프리셋 동급). 프롬프트 원문 아래 §eastern |
| `eastern-interiors` | 동양 내부 4관 — theater(포스터 벽+홍커튼 매표소)·workshop(작업대+가면 스탠드)·teahouse(좌하/우하 테이블+방석)·atelier(대형 거울+한복 랙) | eastern | **생성·배선 완료(2026-07-31)** — 4장 병렬(147~187s) → WebP 275~366KB `lobby-interiors/eastern-{theater,workshop,teahouse,atelier}.webp`, interiorAnchors 오버레이 캘리브(기존 앵커 키 유지 — 소비자 무변경). 실렌더 4관+거울 프리뷰 정박 확인 |
