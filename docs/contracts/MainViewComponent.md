...
> **(2026-07-27 AA3 — 재생 소유권 훅 분리)** MainView(561→278줄)의 센터 재생 로직 전부를 `src/features/stage/useDubCenterPlayback.ts`(320줄)로 **verbatim 추출**(동작 무변경): vodSync 리더/발행/applier(+Y4 힌트) · localMode 진입/복귀 · seekRequest · 베드 슬레이브 · 상시 레이어 · 시사회 · W1 stall 워치독 · onTimeUpdate/onEnded 구간 경계. MainView 는 렌더·HUD 전담 — 이 계약서의 재생 규칙 서술은 이제 훅 파일이 구현 소재다. 실증: 실렌더 beginner-signals 11/11 + 험지 27/27 무회귀.
