...
### § AI 프롬프트 정제 — 개떡 입력 → 카메라시트 (slice1b, 사용자 친화 UI)

프롬프트를 백지에서 쓰게 하지도, 프리셋 틀에 가두지도 않는다. 사용자가 **대충 자유롭게 쓰면 LLM이 Seedance 카메라시트 프롬프트로 확장**한다. (프리셋 칩 방식은 폐기 — 자유도·UX가 이쪽 우위. SSOT 결정: VgenCostAnalysis §4.5)

흐름:
```
사용자 자유 입력(개떡 OK·한글 가능): "예쁜 소녀가 고양이랑 노는 거"
  → [✨ AI로 다듬기] → Edge `refine-vgen-prompt` (LLM, 키 서버 보관·성역)
  → 정제된 영어 카메라시트 프롬프트 (캐릭터 @Image 참조 유지·장면·구도[앵글+무빙]·모션·스타일)
  → 사용자에게 미리보기·수정 허용 → trigger-vgen
```

- **LLM 프로바이더**: 개발=NVIDIA NIM 무료(OpenAI 호환·40 RPM·**production 사용은 유료 라이선스**), 실서비스=`gpt-4o-mini`(이미 보유·요청당 ≈$0.0002·production 제약 없음). 둘 다 OpenAI 호환 → `base_url`·키만 교체.
- **시스템 프롬프트 골자**: "사용자 요청을 Seedance 2.0용 영어 비디오 프롬프트로 확장 — 캐릭터(@Image1.. 참조 유지)·장면·구도(앵글+무빙, 시간대별)·모션·스타일. 프롬프트만 출력." 사용자는 영어·카메라 용어 몰라도 됨.
