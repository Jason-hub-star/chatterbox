...
> - **대극장 화면(로비 v4, 2026-07-09 — 넷플릭스형 개편·주인님 콜)**: 대극장(`rooms`) 내부를 앵커 붙박이(포스터3+리스트)에서 **히어로 자동 캐러셀**(인기 상위 5·6초 순환·도트/좌우화살표·reduced-motion 정지·내부 원화 재활용, 방0=온보딩 CTA) + **무채 미니멀 카드 그리드**(치지직식: 무채 썸네일+방 제목 워터마크·LIVE/대기·좌석 4/6·🔒·프로필 원+제목+장르칩, 색은 장르칩/프로필 원에만 — 콜드스타트 대응)로 교체. `InteriorShell` 탈피(←/Esc 광장 복귀 직접), 생성·예약은 `Modal` 로 보존. 파일 `features/theater/`(RoomCard·TheaterHero·RoomCardSkeleton·genrePresets). **카드 필수 세트(상태·🔒·장르·호스트·인원·점유율)는 §RoomCard 충족** — 아바타도트·씬명·언어뱃지(G-61)·장르개인화(G-58)·마감/대기(G-59)는 명세대로 후속.
> - **검색(LOB-02)**: 클라이언트측 필터(제목·호스트명 `includes`) + **장르 필터칩 토글**(상태 전체/참여가능/LIVE + 장르 6종, `lobby.genre.*` 재활용·무채). 서버 `.ilike`/다중필터(G-60)·preferred_genres 개인화(G-58)는 후속.
...
> - **광장 허브 맵(로비 v2, 2026-07-08 — 주인님 PoC 5회전 검수 채택)**: 데스크톱(md+) 히어로 = 전용 생성 원화 `lobby_plaza_hub_v1`(scene-prompts §로비 v2) 위 **가게 = 기능 입구** 7종(`manifest hub.blocks[].shops`, 기능명 식별자): rooms=대극장(방 목록 스크롤·열린 방 수 뱃지/점등) · social=찻집(최근 함께한 스크롤) · create=공방(생성 폼 포커스) · profile=의상실(/settings) · practice=연습 무대(연습방 /ready) · troupe=극단 회관(COM-01 선점, '준비 중' 토스트) · reserved=예비 점포. 유도 3단(입장 웨이브 1회·휴지 숨쉬기·호버 강점등+포커스 딤) — `HubMap.tsx`+`index.css .hub-*`. 모바일(<768px)은 허브 미표시 + **하단 고정 네비 4탭**(공연·만들기·예약·설정, 주인님 콜) + 기존 배경/스크림 유지. 블록 스트리트 확장 규격은 scene-prompts 참조. E2E 15/15.
