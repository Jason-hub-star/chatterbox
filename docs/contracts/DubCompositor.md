...
> **(2026-07-26 DUB-PART-LOOP — 세그 트림·recordings 계약 확장):** `get-dub-recordings` 응답에 `end_time_ms` 추가(**프로드 배포됨**·클라 `DubRecordingUrl.endTimeMs` 매핑, stale 배포본은 0 폴백=트림 생략). `mixAndMux` 큐 체인이 `atrim=duration=세그길이` 를 adelay 앞에 선행(단일 큐 경로 포함) — **과거 루프 녹음 등 초과 테이크가 다음 세그를 덮는 오염을 합성에서도 격리**(상시 레이어·프리뷰의 durationMs 트림과 동일 규칙). `DubCompositor` cues 에 `durationMs: endTimeMs-startTimeMs` 전달.

> **구현 상태 (2026-07-03, 슬라이스 3b — 음원분리 배경합류, 프로덕션 배포·풀 클릭스루 검증):** G-280 구현 완료. Edge `separate-dub-audio`(호스트·소스 signed URL → fal.ai `fal-ai/demucs` → 비보컬 스템 5종[drums·bass·other·guitar·piano] URL 반환; **DB 무변경 순수 컴퓨트**·크레딧 보호 host gate·`FAL_KEY` 서버 시크릿). `DubCompositor.run()`은 합성 전 `separateDubAudio` 를 소스/녹음 URL 조회와 **병렬** 호출→비보컬 스템 다운로드→`mixAndMux(source, cues, background)` 로 amix(원본 vocals=원어 대사 드롭 → 이중음성 없음). **fal 실증**: mp4 직접 수용(status 200)·분리 품질 실측(원본 mix -19.5dB 음성이 vocals -19.6dB로, 톤이 other로 정확 분리)·$0.0007/초. **합성 실증**: 실 ffmpeg로 `mixAndMux` 배경필터(adelay+amix 비보컬 스템+더빙·`-map 0:v`) 재현 → 유효 mp4(h264 copy+aac). type-check·lint·build·docs:check·deno check PASS. **프로덕션 검증 완료**: `separate-dub-audio` 배포 + 라이브 통합 5/5(호스트200·비호스트403·허위404·CORS `v3b.fal.media` ACAO=`*`). **배포판 풀 클릭스루 5/5**: 배포판(`chatterbox-7r8.pages.dev`) 실브라우저에서 시드 세션(recording·synced) [합성]→"음원 분리 중…"(fal ~42s)→ffmpeg.wasm 합성→세션 `completed`→`dub_outputs` ready→**ffprobe h264+aac 7.0s 유효 mp4**·콘솔에러 0. **defer(ponytail, G-284)**: 스템 Storage 캐시(재합성 재과금 방지)·2-stem/서버병합(대역폭 5×↓)·긴 클립 fal 큐/웹훅·노래 보컬 손실→AudioShake 승급.
>
