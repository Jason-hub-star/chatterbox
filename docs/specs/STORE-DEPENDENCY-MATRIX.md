...
| `VoiceRecording.md` | `userStore.session`, `audioStore.micDeviceId` | 없음 | ROOM-28 음성 전용 녹음. **신규 store 금지** — 상태는 `useRoomRecording` 훅 로컬. `stageStore.backgroundUrl` 은 음성 모드에서 읽지 않는다 |
