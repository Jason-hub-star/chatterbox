...
| `POST /functions/v1/start-dub-compositing` | `Host` | `{ dub_session_id, idempotency_key }` | `{ output_id, status: 'compositing' }` | All tracks synced + consent + quota + flag gate; start compositor; audit | [[DubCompositor]], [[DATA-SCHEMA]] |
