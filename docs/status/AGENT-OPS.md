...
| ISS-03 | 골 사다리 인계 | **8골 사다리(`docs/goals/GOAL-LADDER.md` = 상태 SSOT) — G1~G4 DONE(G4=자막편집: mov_text/VTT 소프트 자막, 프로드 클릭스루 13/13 — 배포 완료).** 다음: G5 관객 투표 — entrypoint=GOAL-LADDER G5 행 근거 매트릭스(`API-SURFACE:126` submit-viewer-poll 계약 기존재·갭=polls/poll_responses 마이그 2뿐·재사용 broadcastData/check_rate_limit/공유 Modal). read first: ①GOAL-LADDER ②GAP-MATRIX 진행 로그 2026-07-12 최상단. blocker: none(R2 실키는 이 머신에 없음 — 로컬 E2E 는 R2 무관 경로로). first verify: `npm run check:all` | 2026-07-12 | HANDOFF |
