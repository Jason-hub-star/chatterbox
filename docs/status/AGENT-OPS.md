...
| ISS-07 | 골 사다리 G9 인계 | **G9 더빙 녹음 체감(`goals/GOAL-dub-recording-tangible.md`) P1~P4 완주+bepo 완료(2026-07-18)** — 레벨미터+무음경고·녹음 로컬모드+즉시 미리보기(Web Audio·ffmpeg 불필요)·누적 시사회 전원(get-dub-recordings v13 멤버 완화·SEC-RA-1 dub_screening)·내차례 배너+±200ms 캘리브레이션(submit-dub-track v10·합성 동일 적용). CF `index-DpVmQqc7.js` 별칭·배포판 스모크 8/8. **잔여(후속 후보)**: 늦은 입장자 시사회 하트비트 · 비호스트 seek 시 시사회 오디오 재스케줄 · 멀티트랙 영상 편집기(별도 대설계 골) · DUB-FRICTION 잔여(다인 진행 타임라인·합성 ETA·소스언어 사후변경·휠 편집 발견성). read first: 브리프 §7. 첫 검증: `npm run check:all`. blocker: none. | 2026-07-18 | DONE |
