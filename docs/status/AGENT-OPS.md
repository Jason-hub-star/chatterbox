...
| ISS-03 | 골 사다리 인계 | **8골 사다리(`docs/goals/GOAL-LADDER.md` = 상태 SSOT) — G1·G2 DONE, G3 인앱녹화 A·B·C·D 완료(통합 17/17·레코더 하네스 8/8·2탭 실렌더 13/13·check:all 그린).** next entrypoint=`docs/goals/GOAL-g3-room-recording.md` §5 — **남은 작업 E 하나: ship-live**(db push 마이그 `20260712120000`+Edge 5종 deploy+livekit-token(F-1)+CF Pages → 프로드 라이브 녹화 1건 실측이 G3 DONE 게이트). read first: ①GOAL-LADDER ②GOAL-g3 ③GAP-MATRIX 진행 로그 2026-07-12 4행. blocker: **배포 승인 게이트 대기**(주인님 신호). first verify: `npm run check:all` 후 ship-live 스킬. E2E 하네스 재사용: scratch `g3d-2tab-e2e.mjs`(BASE 만 프로드로) | 2026-07-12 | HANDOFF |
