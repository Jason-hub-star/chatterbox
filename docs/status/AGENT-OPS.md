...
| ISS-03 | 골 사다리 인계 | **8골 사다리(`docs/goals/GOAL-LADDER.md` = 상태 SSOT) — G1 문서리팩토링·G2 F-패스 DONE, G3 인앱녹화 ACTIVE(A 스키마+B Edge5 완료·통합 17/17, 커밋 6a0941b·4df5341·d123a63).** next entrypoint=`docs/goals/GOAL-g3-room-recording.md`(§0 방식결정·§5 페이즈 — 다음 작업 C: `src/features/stage/stageRecorder.ts` 무대 캔버스 합성+WebAudio 믹스 레코더, 이어서 D: 동의 모달·⏺ 배선·REC·재생 UI). read first: ①GOAL-LADDER(골별 스킬·문서 매트릭스) ②GOAL-g3 ③GAP-MATRIX 진행 로그 2026-07-12 3행. blocker: none. first verify: `npm run check:all`(그린 기준선) 후 C 착수. **배포 대기열(승인 게이트)**: livekit-token(F-1 닉네임)·G3 마이그 db push+Edge 5종·CF Pages(G2 프론트+ISS-02 이모트 미배포분) — G3-D 완료 시 ship-live 일괄 권장 | 2026-07-12 | HANDOFF |
