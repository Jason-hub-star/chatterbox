...
| ISS-03 | 골 사다리 인계 | **8골 사다리(`docs/goals/GOAL-LADDER.md` = 상태 SSOT) — G1·G2·G3 DONE, G4 BUILT(자막편집: Edge `update-dub-segment-text`+mov_text/VTT 소프트 자막, 통합 10/10·E2E 11/11 — 배포 대기: Edge 1+프론트, 프로드 클릭스루=bepo 시).** 다음: ①G4 배포(bepo — 승인 게이트) ②G5 관객 투표 — entrypoint=GOAL-LADDER G5 행 근거 매트릭스(`API-SURFACE:126` submit-viewer-poll 계약 기존재·갭=polls/poll_responses 마이그 2뿐·재사용 broadcastData/check_rate_limit/공유 Modal). read first: ①GOAL-LADDER ②GAP-MATRIX 진행 로그 2026-07-12 최상단. blocker: none(R2 실키는 이 머신에 없음 — 로컬 E2E 는 R2 무관 경로로). first verify: `npm run check:all` | 2026-07-12 | HANDOFF |
