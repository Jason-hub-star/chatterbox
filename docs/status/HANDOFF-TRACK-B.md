...
- [ ] **Toast/피드백 UI** — `<ToastHost/>` (A-SEAM-1 채널 위)
- [ ] **Progress/ETA 바** — VGEN·DUB (A-SEAM-2 데이터 위)
- [ ] **반응형 레이아웃 + 터치 롱프레스 리액션 + 무대 동적 크기** — 앱 전체 `sm:`/`xl:` 0회, 무대 `stageLayout.ts` SLOT_PX 고정 120px, 리액션 우클릭 전용→터치 불가 (P-5)
- [ ] **모바일 뷰어 레이아웃** — MobileViewer (A-SEAM-4 권한 위 · 계약 `contracts/MobileViewer.md`)
- [ ] **다국어 번역 채우기** — en/ja 나머지 키 (A-SEAM-5 구조 위)
- [ ] **모달 일원화**(P-4 focus-trap·Esc·복귀포커스) · **다크 elevation 그림자**(index.css 미배선, 깊이 전부 border) · **라이트모드**
- ~~대비 교정~~ — 취소(WCAG 실측 muted 7.2:1, AA 통과 — 정찰 오탐).

우선순위 사다리는 `UX-GAPS §3` 참조: **P0**=Toast·Progress/ETA·조인 취소버튼 → **P1**=반응형+터치·모바일뷰어·다국어 → **P2**=모달·그림자·라이트모드.
