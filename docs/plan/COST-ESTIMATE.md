...
## 현행 as-built 스택 비용 (2026-07, VGEN 미구현)

> **범위 구분(중요):** 아래 "DAU별 월 비용 추정"·"핵심 비용 드라이버"는 **VGEN(fal.ai Seedance) 포함 전체 계획 스택** 기준이며 그 경우 **fal.ai 가 77~90% 지배**(G-270). 하지만 **현재 구현·배포된 스택은 방(LiveKit 오디오+DataChannel)·실시간 연기·더빙(STT+클라 ffmpeg.wasm 합성)뿐 — VGEN 미구현.** 이 as-built 스택의 비용 곡선은 아래처럼 **완전히 다르다.**

**핵심: 아바타=DataChannel(영상 트랙 없음)·합성=클라 ffmpeg.wasm·저장=R2(egress $0) → 대역폭/egress 지뢰가 구조적으로 없음.** 유일한 스케일 변수 = **LiveKit 연결분(참가자×분)**. STT/분리/스토리지/호스팅은 marginal.

가정: 유저별 방 체류시간이 LiveKit 을 좌우. LiveKit $0.0005/참가자분, STT=Groq $0.00067/분.

| 구간 | 방 사용 | LiveKit | STT+분리 | Supabase | R2·호스팅 | **월 합계** |
|---|---|---|---|---|---|---|
| **MVP**(베타) | 30 DAU×30분/일 ≈ 27K분 | ~$25(기본플랜) | ~$2 | $25 | ~$1·$0 | **~$50** |
| **소규모** | 300 DAU×40분/일 ≈ 360K분 | ~$180 | ~$25 | ~$40 | ~$5·$0 | **~$250** |
| **성장** | 3,000 DAU×50분/일 ≈ 4.5M분 | ~$2,250 | ~$245 | ~$100 | ~$30·$20 | **~$2,650** |

→ **MVP ≈ Supabase $25 + LiveKit 기본 $25 = 월 ~$50.** 스케일할수록 **LiveKit 이 75~85% 지배**(연결분 선형). **VGEN 출시 시점부터** 위 fal.ai 지배 모델로 전환된다.

**더 나은 선택지(2026-07 조사·Opus 판정):**
- **STT: OpenAI Whisper → Groq(whisper-turbo)** — $0.006→$0.00067/분 **9×↓**·더 빠름. 더빙 전사 호출만 교체(G-269 저비용 옵션). JP 품질만 전환 전 실측.
- **프론트 배포: Cloudflare Pages(무료·무제한 대역폭)** > Vercel($20). 미배포라 지금 선택하면 됨.
- **대용량 미디어 → R2(egress $0)** 유지(이미 스펙). S3 egress 지뢰 회피.
- **3b 음원분리: Replicate Demucs(~$0.045/곡, 입증)** — fal 잔액 차단·공개가격 없음의 대안.
- **Supabase·LiveKit 유지(이전 기각):** Neon 은 Auth/Storage/Edge/Realtime 다 잃고 재작성(월 $10 절약 위해 통합 포기 X), 100ms 는 LiveKit 보다 비싸고 아바타/텔레프롬프터 sync 가 전부 LiveKit DataChannel. 성장기(월 100~400K분↑)엔 **자체호스팅 LiveKit(Hetzner 고정 ~$50~200)** 재검토.

**미검증:** fal Demucs 공개가격 없음(콘솔 직접 확인)·Groq JP STT 품질(전환 전 실측).
