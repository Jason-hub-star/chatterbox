...
- [ ] **모바일 뷰어 레이아웃** (A-SEAM-4 위에) — ⚠️ **표현만으론 못 닫음**: 뷰어로 입장할 문(초대코드 `verify-invite`/`accept-invite` Edge·viewer 조인·anon auth)이 미구현(A-SEAM-4 defer 범위) → 레이아웃 선작업은 죽은 코드. 뷰어 입장 기능 슬라이스와 함께 착수.
