...
- [x] **SEC-RES-1** (Med) 예약 생성 rate-limit + 무권한 초대 차단 — `create-reservation`에 `check_rate_limit('reservation:<userId>', 20, 86400)` 일일 캡 추가(SEC-1/4/6 프리미티브 재사용·유효요청 통과 후 계수) + invitee 를 "내 최근 함께한 동료"(공유 room_participants 이력 ∩ 미삭제)로 제한(임의 uuid → 관계 없는 유저 알림 주입 차단). 응답 `notified` 를 실제 발송수(fellowIds)로 정정. 두 수정 모두 `create-reservation` 한 곳. **실측:** deno check clean(RPC 로직은 SEC-1서 격리 postgres 실측·동일 프리미티브). ⏳ 배포 대기(supabase functions deploy create-reservation).
- [x] **SEC-RES-2** (Low) `list-recent-people` excludeRoomId 미검증 — 클라 공급 room_id 를 `roomIds.includes(excludeRoomId)`(내 최근 참가 이력)로 게이트 후에만 제외 조회(`list-recent-people`). 정상 사용=내가 있는 방 제외라 무손상, 임의 방은 무시 → 추론 차단(추가 쿼리 0, roomIds 재사용). **실측:** deno check clean. ⏳ 배포 대기(supabase functions deploy list-recent-people).
