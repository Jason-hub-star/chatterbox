...
| G5 | V-5 관객 투표 | 생성→투표→집계 2탭 동기(뷰어 포함) | 통합 + 2탭 E2E | **BUILT**(2026-07-12: 마이그 `20260712150000` polls·poll_responses + Edge 3(create-poll·set-poll-status·submit-viewer-poll) + PollBar/HostConsole 섹션/pollStore/'poll' 릴레이 — 통합 **26/26**(403/400/409·RLS·upsert 1인1표·reveal 스냅샷·레이트리밋 429) + 2탭 E2E **11/11**(A 생성→B 라이브 수신·투표→A 총계→reveal percent→리로드 복원→close 소멸). 배포=bepo 대기) |
...
| G6 | supabase-slice-verify(E2E) · cf-pages-deploy-verify · bepo | `contracts/AudioMixer.md`(BGM=HTMLAudioElement MUST) · `contracts/ReactionWheel.md §비주얼`(MAX_LOTTIE_FLOATS=8·강등 패턴) · `design/DESIGN-TOKENS.md §6·8` | 파티클=기존 `GlowMotes`(캔버스) 확장 · SFX 타이밍=EmoteGlyph 재생 훅 · reduced-motion 관례 8곳 기존재 · autoplay 정책=첫 제스처 게이트 필수 |
