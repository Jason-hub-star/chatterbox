...
| G9 | 더빙 녹음 체감(설계) | 마이크 실시간 시각화 + 녹음 즉시/누적 영상 미리보기(ffmpeg 불필요) + SSOT 갭(내차례 배너·±200ms 캘리브레이션) | 헤드리스 Chrome(가짜 마이크) 실렌더 + `check:all` | **설계 PENDING**(2026-07-17: 브리프 `GOAL-dub-recording-tangible.md` — DubRecorder.md SSOT 대조·기술설계(Web Audio AnalyserNode 레벨미터·AudioBufferSourceNode offset 스케줄 미리보기)·웹 레퍼런스 8종 조사 완료·구현 미착수(주인님 "설계문서만" 선택). 멀티트랙 영상 편집기는 범위 밖 별도 대설계) |
