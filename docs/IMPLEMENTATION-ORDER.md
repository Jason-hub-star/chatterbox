...
## 빌드 전략 — 세로 슬라이스 (기능 우선, UI는 뒤)

ChatterBox는 **세로(vertical) 슬라이스**로 쌓는다. 한 기능을 `DB → RLS → Edge Function → store → 최소 UI`까지 **끝까지 뚫어 "실제로 도는" 상태**로 만든 뒤 다음으로 넘어간다. 가로로(모든 화면의 UI를 먼저) 쌓지 않는다.

- **기능·데이터 흐름이 먼저.** 각 슬라이스는 검증(마이그레이션 적용·psql RLS 실측·API/E2E)까지 끝내고 커밋한다. "확인 대신 추측" 금지.
- **비주얼 UI는 뒤로 미룬다.** 현재 페이지는 디자인 토큰(`design/DESIGN-TOKENS.md`)만 얹은 **기능 셸**(리스트+버튼)이다. 실제 무대 레이아웃·컴포넌트·모닥불 비주얼(`DESIGN-DIRECTION.md`)은 **MILESTONES Phase 3(묵대 완성)** 에서 구현한다. 설계는 SSOT에 확정돼 있고 구현만 대기 중 — "디자인 무시"가 아니다.
- **이유**: 데이터 흐름이 확정되기 전 UI를 완성하면 갈아엎게 된다. 토큰이 이미 깔려 있어 나중 리스타일 비용은 낮다(색·간격 일관, 레이아웃만 얹으면 됨).
- **미룬 것은 명시한다.** 슬라이스에서 뺀 항목은 `ponytail:` 주석 + 후속 슬라이스로 표기(예: Phase 2에서 초대링크·비밀번호 방·kick/mute·로비 Realtime 등 defer).
