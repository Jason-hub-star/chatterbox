...
[엣지] create-avatar-job: getAppUser → avatar_jobs insert(queued) → 업로드 PNG signed GET URL
      → Modal 웹엔드포인트 POST(spawn, body에 trigger_secret) → provider_call_id+첫 analyzing lease(75분) 저장·status=running
[Modal] submit(fastapi_endpoint): 시크릿 검증 → png 다운로드→볼륨 → run_pipeline.spawn
        run_pipeline: generate_inputs → decompose → build_rig → publish
          각 단계 사이 renew_avatar_job_lease RPC(service_role): phase=analyzing→cutting→rigging→finishing + bounded lease 갱신
          publish(bake_storage_avatar.py): project.json + parts/*.webp → Storage avatars/<job>/
          → complete_avatar_job RPC(status=done, result_project_url, completed_at; active row만 전이)
[DB] pg_cron(5분): lease 만료 running 또는 15분 초과 queued → fail_avatar_job(orchestrator_timeout)
...
- **잡 수명**: `provider_call_id`는 Edge가 저장하고, 단계 lease가 하드킬·원격 예외의 DB 미갱신을 막는다. 늦게 살아난 워커는 terminal 잡을 `done`으로 되살릴 수 없다. watchdog은 GPU를 취소하지 않고 사용자 상태를 실패로 종결하므로, 볼륨에 남은 리그는 기존 구조 발행 절차로 복구 가능하다.
...
## 입 시스템: states vs wide_grow (로스터 성공 레시피)

발화 시 입이 자연스러운지는 **`build_rig`가 뽑는 입 모드**가 좌우한다(2026-07-16 실측 정리·이 문서가 흩어진 기록의 허브).

- **`states`(성공·검증됨)** — `mouth_state_small`·`mid`·`wide` **3장을 각각 자연 비율로 그려두고** 웹캠 개구가 그중 하나를 선택/블렌드. **우로·미미**가 이 방식(+각도 `ParamAngleY` 대응). **유키**는 레거시(states 3장·각도 무). → 늘림 아티팩트 없음.
- **`wide_grow`(결함 이력)** — `mouth_state_wide` **1장을 `ParamMouthOpenY`로 0→끝까지 스트레치**(small/mid 없음). 과다개구 + 세로 늘어남. 이전에도 결함으로 걸린 방식(아래 `99c74a83`).

**캘리브레이션 기록**(SSOT=`scripts/qa-mouth-lips.mjs` 주석): 진폭 `AMP_MAX_RATIO=0.13`(열린입/얼굴 높이) = n=5 실측 **정상 10.6%(`1a30`) vs wide_grow 결함 17.5%(`99c74a83`)** 사이. 립 소실 축=`poon995`(small 0.19 결함 vs mid 0.64), 캐비티 축=`4909f992`(0.58) vs 정상 mimi(0.03). → **미학 드리프트("입 높음")는 기하·VLM 자동 판정 불가**(위 §UI/UX 노트) — 진폭 과대는 잡지만 "자연스러움"의 최종심은 사람 눈.

**예방**: `qa-mouth-lips`가 `deploy-avatar`·`publish-avatar-job`에 게이트로 걸림. 비상 우회 `QA_MOUTH_SKIP`은 이제 `docs/status/qa-mouth-bypass.tsv`에 id·시각·사유 durable 기록(`scripts/lib/qa-bypass-log.mjs`) — 우회 배포된 아바타 추적·재게이트용.

**airi 사례(2026-07-16)**: 로스터 유일 wide_grow. 19% 과다개구 + `mouth_state_wide absolute primary` 계약 태그 누락으로 `QA_MOUTH_SKIP` 우회 배포됐던 것을 실측 발견. **스톱갭** = 키폼+스프라이트 0.55 스쿼시(→10.6%, 게이트 통과·렌더 자연화) + 태그 보정, 프로드 재배포. **정공** = states 방식으로 재생성(우로/유키/미미처럼). 생성 시 모드 선택 로직은 Vtube `chatterbox-avatar-forge`(`build_rig`). 루프 상세=`docs/goals/GOAL-mouth-natural.md`.
