...
5. **렌더러/페이지는 범용(이름만 rig)** — `RigAvatar`(`src/lib/pixi/rig`)·`AvatarInspectorPage`(`/avatar-inspect`)는 아무 rig 나 렌더. DEV 는 `window.__rigAvatar` 노출. "aria" 는 캐릭터명이지 코드 전용 아님.
...
- 성공 사례: akane(47 parts·6/6)·ruby(45·6/6)·yuki(44·6/6) 배포+실렌더.
...
- Storage 레이아웃: `avatars/<id>/project.json` + `avatars/<id>/parts/*`. 캐릭터 "aria/akane/ruby/yuki" 는 실제 캐릭터.
