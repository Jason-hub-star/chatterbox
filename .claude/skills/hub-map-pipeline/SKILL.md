...
고래 매팅(육안 폴리곤 — 어둡기 자동검출은 밝은 별빛 텍스처에 실패)+TELEA inpaint+PixiJS MeshRope 유영까지 구현·검증했으나 **주인님 판정 "너무 어색" → 전면 롤백**(원화의 정적 고래 유지). 자산은 계보 폴더(`~/Documents/채터박스/v2/whale-*.png`·`*_sky.png`)와 git 이력(`PlazaAmbient.tsx`)에 보존. 재시도 시 개선 가설: 회화 스타일 스프라이트의 메시 굴곡이 "종이 인형" 느낌을 준 것 — 굴곡 없이 초저속 드리프트+스케일 호흡만, 또는 i2v 시네마그래프 검토.
