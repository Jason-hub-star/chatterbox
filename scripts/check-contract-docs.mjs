...
if (failures.length === 0) {
  const docs = walkFiles('docs', (rel) => rel.endsWith('.md'));
  ...
  metrics.createTables = (read('docs/DATA-SCHEMA.md').match(/\bCREATE\s+TABLE\b/gi) ?? []).length;
  ...
  for (const row of unresolvedP1Pending(read('docs/DATA-SCHEMA.md'))) {
